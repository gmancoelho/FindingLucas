...
public extension Date {
  ...
  static var startDate = Date(timeIntervalSince1970: 0)
...
public extension Date {
  ...
  public func validateYears(year: Int) -> Bool {
    let date = Calendar.current.date(byAdding: .year, value: -year, to: Date())
    if self <= date! {
      return true
    }
    return false
